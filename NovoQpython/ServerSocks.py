...
class socksocket(socket.socket):
    ...
    def __negotiatehttp(self, destaddr, destport):
        ...
        if self.__headers:
            ...
            if self.__proxy[6] is not None:
                headers += ['User-Agent: ', unicodestring1(self.__proxy[6]), '%s' % self.__newline]
        if self.__proxy[4] != None and self.__proxy[5] != None:
    ...
    def connect(self, destpair):
        if type(destpair) not in (list, tuple) or len(destpair) < 2 or not isinstance(destpair[0], askstring) or type(destpair[1]) != int:
